...
public final class XMLLookParser {
	 ...
	 * @param xml Ruta del documento XML.
	 * @param field Campo a obtener.
	 * @param p Propiedades
	 * @param pke Clave privada.
	 */
	...
	public Properties parse() throws XMLException {
		...
		try(final InputStream in = new ByteArrayInputStream(this.xml.getBytes(StandardCharsets.UTF_8))) {
			...
			if (this.field == null) {
				final Node page = doc.getElementsByTagName("signaturePosition").item(0); 
				if (page != null) {
					this.prop.setProperty("imagePage", page.getTextContent()); 
				}
				final Node rect = doc.getElementsByTagName("Rect").item(0); 
				if (rect != null) {
					setRectProp(rect);
				}
			}
			...
			final Node params = doc.getElementsByTagName("params").item(0); 
			if (params != null) {
				setParams(params);
			}
			final Node background = doc.getElementsByTagName("Background").item(0); 
			if (background != null) {
				setBackgroundImage(background);
			}
			final Node foreground = doc.getElementsByTagName("Foreground").item(0); 
			if (foreground != null) {
	...
	private void setRectProp(final Node rect) {
		...
		if (rectAttrib != null && rectAttrib.getLength() == 4) {
			final String x0 = rectAttrib.getNamedItem("x0").getTextContent(); 
			final String y0 = rectAttrib.getNamedItem("y0").getTextContent(); 
			final String x1 = rectAttrib.getNamedItem("x1").getTextContent(); 
			final String y1 = rectAttrib.getNamedItem("y1").getTextContent(); 

			this.prop.setProperty("signaturePositionOnPageLowerLeftX" , x0); 
			this.prop.setProperty("signaturePositionOnPageLowerLeftY" , y0); 
			this.prop.setProperty("signaturePositionOnPageUpperRightX" , x1); 
			this.prop.setProperty("signaturePositionOnPageUpperRightY" , y1); 
	...
	private void setParams(final Node params) {
		...
		for (int i = 0; i < paramsList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
	        	...
	        	else if (element.getNodeName().equals("reason")) { 
	        		this.reason = element.getTextContent();
	        	}
	        	else if (element.getNodeName().equals("contactInfo")) { 
	...
	private void setBackgroundImage(final Node background) throws DOMException, IOException, XMLException {
		...
		for (int i = 0; i < backgroundList.getLength(); i++) {
			...
			if (backImage != null && backImage.getNodeName().equals("image")) { 
				...
				int posX = 0;
				int posY = 0;
				final String encodeType = imageElement.getAttribute("encodeType"); 
				for (int j = 0; j < list.getLength(); j++) {
					final Node node = list.item(j);
					if (node.getNodeType() == Node.ELEMENT_NODE) {
						final Element element = (Element) node;
			        	if (element.getNodeName().equals("data")) { 
			        		if (encodeType.equals("uri")) { 
								try {
			        	...
			        	if (element.getNodeName().equals("imageSize")) { 
			        		final NamedNodeMap imageSize = element.getAttributes();
			        		if (imageSize != null && imageSize.getLength() == 2) {
			        			width = Integer.parseInt(imageSize.getNamedItem("width").getTextContent()); 
			        	...
			        	if (element.getNodeName().equals("position")) { 
			        		...
			        		if (imageSize != null && imageSize.getLength() == 2) {
			        			...
			        			posY = Integer.parseInt(imageSize.getNamedItem("y").getTextContent()); 
			        		}
			        	}
					}
				}
	...
	private void setForeground(final Node foreground) throws DOMException, IOException, XMLException {
		...
		for (int i = 0; i < foregroundList.getLength(); i++) {
			...
			if (foreItem != null && foreItem.getNodeName().equals("image")) { 
			        	...
			        	if (element.getNodeName().equals("data")) { 
			        		if (encodeType.equals("uri")) { 
								try {
									...
									LOGGER.severe("Error extrayendo propiedades del frente: " + e); 
									throw new XMLException("Error extrayendo propiedades del frente: " + e, e); 
								}
			        		}
			        		else if (encodeType.equals("base64")) { 
			        			im = getImageFromBase64(element.getTextContent());
			...
			if (foreItem != null && foreItem.getNodeName().equals("text")) { 
				setText(foreItem);
			}
		}
	}
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
	        	...
	        	if (element.getNodeName().equals("properties")) { 
	        		...
	        		if (textPropAttrib != null && textPropAttrib.getLength() == 2) {
		        		...
		        		y = Integer.parseInt(textPropAttrib.getNamedItem("y").getTextContent()); 
	        		}
	        	}
	        	else if (element.getNodeName().equals("SignatureInfos")) { 
	        		text = setSignatureInfo(element);
	        		paintText(text, x, y, color, fontSize);
	        	}
	        	else if (element.getNodeName().equals("ArbitraryText")) { 
	...
	private void setArbitraryText(final Node arbitrary) {
		...
		for (int i = 0; i < arbitraryList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
	        	...
	        	if (textItem.getNodeName().equals("textItem")) { 
	...
	private String setSignatureInfo(final Node signatureInfos) {
		...
		for (int i = 0; i < signatureInfoList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				final Element element = (Element) node;
	        	if (element.getNodeName().equals("signatureInfo")) { 
	...
	private static Color getTextColor(final NamedNodeMap textPropAttrib) throws XMLException {
		...
		final String rgbString[] = textPropAttrib.getNamedItem("color").getTextContent().split(" ");  //$NON-NLS-2$
		for (int j = 0 ; j < 3 ; j++) {
